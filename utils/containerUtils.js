// Container number utility functions
...
export function calcCheckDigit(containerNumberWithoutCheckDigit) {
  let sumOfProduct = 0;
...
export function validateContainerNumber(containerNumber) {
  // Check length
  if (containerNumber.length !== 11) {
    return { isValid: false, error: `Length should be 11 characters (current: ${containerNumber.length})` };
  }
  
  // Check format pattern: 3 letters + 1 letter (J,U,Z) + 6 digits + 1 digit
  const pattern = /^[A-Z]{3}[JUZ][0-9]{6}[0-9]$/;
  if (!pattern.test(containerNumber)) {
    return { isValid: false, error: "Format: AAABNNNNNN (3 letters + J/U/Z + 7 digits)" };
  }
  
  // Validate check digit
  const containerWithoutCheck = containerNumber.substring(0, 10);
  const providedCheckDigit = parseInt(containerNumber.charAt(10));
  const calculatedCheckDigit = calcCheckDigit(containerWithoutCheck);
  
  if (providedCheckDigit !== calculatedCheckDigit) {
    return { 
      isValid: false, 
      error: `Check digit mismatch. Expected: ${calculatedCheckDigit}, Got: ${providedCheckDigit}` 
    };
  }
